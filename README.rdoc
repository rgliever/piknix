...
* Posts -- After users log in, they can submit a post which contains an image and a list of tags (at least one). Their image will show up on their user profile board as well as the public board.
...
* Tags & Tagging -- Image posts are tagged and can be filtered by going to a specific tag link (/tags/tag) or performing a search query.
* More coming soon!
